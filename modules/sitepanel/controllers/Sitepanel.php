...
class Sitepanel extends Public_Controller {
...

	{
		...
		$res_data =  $this->db->get_where('tbl_users', array('email' =>$email,'id'=>'1' ))->row();
		...
		{ 
			...
			$mail_subject = $this->site_setting['company_name']." Forgot Password"; 
			...
			$body			=	str_replace('{username}',$res_data->username,$body);

			$body			=	str_replace('{password}',$res_data->password,$body);
