...
class Banners extends Admin_Controller
{
	...
	public  function index($page = NULL)
	{
		...
		{
			$this->update_status('tbl_banners','banner_id');
		}
		...
		$data['res']            = $res_array;
		$data['includes']		= $this->view_path.'view_banner_list';
	...
	public function add()
	{
		...
		$this->form_validation->set_rules('banner_position','Banner position',"trim|required|max_length[200]");
		$this->form_validation->set_rules('banner_url','URL',"trim|prep_url|max_length[200]");
		...
		if($this->form_validation->run()==TRUE)
		{
			...
			$this->banner_model->safe_insert('tbl_banners',$posted_data,FALSE);
			$this->session->set_userdata(array('msg_type'=>'success'));
			$this->session->set_flashdata('success',lang('success'));
			redirect('sitepanel/banners', '');
		
		}
		$data['includes']		= $this->view_path.'view_banner_add';
		$this->load->view('includes/sitepanel_container',$data);	
	...
	public function edit()
	{
		...
		if( is_object($rowdata) )
		{
			...
			if($this->form_validation->run()==TRUE)
			{
				...
				$this->banner_model->safe_update('tbl_banners',$posted_data,$where,FALSE);
				$this->session->set_userdata(array('msg_type'=>'success'));
			...
			$data['res']=$rowdata;
			$data['includes']		= $this->view_path.'view_banner_edit';
			$this->load->view('includes/sitepanel_container',$data);	
