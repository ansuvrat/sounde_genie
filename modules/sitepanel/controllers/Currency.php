...
class Currency extends Admin_Controller
{
	...
	{		
		...
		{			
			$this->update_status('tbl_currencies','currency_id');			
		}
	...
	{		  
		...
		$this->form_validation->set_rules('code','Currency Code',"required|max_length[3]|unique[tbl_currencies.code='".$this->db->escape_str($this->input->post('code'))."' AND status!='2']");
		...
		if($this->form_validation->run()==TRUE)
		{
		    ...
		    $this->currency_model->safe_insert('tbl_currencies',$posted_data,FALSE);									
	...
	public function edit()
	{
		...
		$this->form_validation->set_rules('image1','Image',"file_allowed_type[image]");
				...
				if($this->form_validation->run()==TRUE)
				{
					...
					$this->currency_model->safe_update('tbl_currencies',$posted_data,$where,FALSE);						
					$this->session->set_userdata(array('msg_type'=>'success'));				
