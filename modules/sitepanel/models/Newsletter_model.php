...
class Newsletter_model extends MY_Model{
	...
	{	
		...
		$nwsmem               =  $this->db->escape_str(trim($this->input->get_post('nwsmem',TRUE)));
		$newsletter_groups_id  =  $this->input->get_post('newsletter_groups_id',TRUE);
		$where_condtion  ="status != '2' ";
        if($keyword != ''){
			$where_condtion.=" AND  ( subscriber_email like '%$keyword%' OR  subscriber_name like '%$keyword%' OR  last_name like '%$keyword%' OR CONCAT_WS(' ',subscriber_name,last_name) LIKE '%".$keyword."%'  ) "; 

		}
		if($nwsmem != ''){
			$where_condtion.=" AND is_reg_member='$nwsmem' ";
		}
