...
function create_form_hidden($remove='')
{
   $_ci =& get_instance();
   $a1 =  (array) $_ci->input->get();
   $a2 =  (array) $_ci->input->post(); 
   $final_array =  array_merge($a1,$a2);
   $get_data = @array_filter($final_array);
	...
	if($remove!='')
	{
		...
		foreach ($remove as $rm)
		{		
			if (is_array($get_data) && array_key_exists($rm,$get_data))
			{
			  unset($get_data[$rm]);
			}
		}
